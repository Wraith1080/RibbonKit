...
- The test suite has 237 green logic tests plus one visual test covering 62 approved images as of 2026-08-10. Phase 7 merge/modal invariants, prefix-free/typeable KeyTip resolution plus explicit custom-content discovery/invocation, disabled-target blocking, native toggle Click/Command semantics across Backstage and application-menu panes, compact checkbox/radio/text input discovery with Header-derived labels, collapsed-group preservation for editor/picker activation, and app-owned Showcase appearance-preference serialization are covered, alongside the existing theme/template/RTL/visual contracts; important remaining gaps include broader customization round trips and remaining reduction-algorithm cases. Touch mode remains a pre-freeze API consideration, not committed scope; the current templates need a semantic density-metric layer before a complete toggle is viable.
