# Writer consumer-friction evidence

> Original dated observations retained on 2026-09-08. Status words and instructions
> describe their recorded checkpoint and may be superseded. Use the
> [current friction index](../12-RIBBONKIT-WRITER-CONSUMER-FRICTION-LOG.md) and
> [AGENTS.md](../../AGENTS.md) for present work. No entry grants runtime authorization.
...
- **Consumer goal:** select a printer while seeing the fixed preview of the intended output.
- **Reproduction and evidence:** `System.Windows.Controls.PrintDialog.ShowDialog()` opened the current Windows print
  ...
  the document later through `PrintDocument`; it offers no document input for that Windows preview pane.
- **Friction:** the OS message contradicted Writer's working preview and made the accepted preview/print contract look
...
- **Reproduction and evidence:** Microsoft Print to PDF worked for text-only documents but terminated Writer when the
  document contained a picture. Writer was submitting the preview's in-memory `FixedDocumentSequence` for printing, so
  WPF serialized already-packaged bitmap resources a second time into the device XPS spool.
- **Current app-owned correction:** keep the fixed document for stable preview/navigation, but print from the same
  snapshot's isolated flow-document clone. Bitmap resources are then serialized once into the spool; page
